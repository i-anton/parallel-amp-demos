...
void logic::parallel(PixelVector& state, int field_size, PixelVector& state_double, int iter) {
...
void logic::parallel_branchless(PixelVector& state, int field_size, PixelVector& state_double, int iter) {
	using namespace concurrency;
	array_view<PixelData, 2> data_out(field_size, field_size, &(state[0]));
	data_out.discard_data();
	array_view<PixelData, 2> new_buffer(field_size, field_size, &(state_double[0]));
...
void logic::parallel_branchless_const(PixelVector& state, int field_size, PixelVector& state_double, int iter) {
	using namespace concurrency;
	array_view<PixelData, 2> data_out(field_size, field_size, &(state[0]));
	data_out.discard_data();
	array_view<PixelData, 2> new_buffer(field_size, field_size, &(state_double[0]));
